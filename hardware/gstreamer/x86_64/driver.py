...
third_party/x86_64/launch_rtsp_server -p {} -m /cam "shmsrc is-live=true socket-path=/tmp/camera_I420_{} do-timestamp=1 ! video/x-raw{},framerate=(fraction)0,format=I420 ! queue ! x264enc speed-preset=1 threads=8 tune=zerolatency sliced-threads=false bitrate={} ! h264parse ! rtph264pay pt=96 name=pay0 {}" &'''
...
def get_rtsp_sink_daemon_impl(idx, out_param, bitrate, audio, port):
    return RTSP_SINK_DEAMON_TEMPLATE.format(port, idx, out_param, int(bitrate / 1024), audio)
