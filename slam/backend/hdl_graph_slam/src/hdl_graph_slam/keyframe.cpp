...
KeyFrame::KeyFrame(const uint64_t& stamp, const Eigen::Isometry3d& odom, double accum_distance, const pcl::PointCloud<PointT>::ConstPtr& cloud) : stamp(stamp), odom(odom), accum_distance(accum_distance), cloud(cloud), node(nullptr) {}
...
void KeyFrame::save(const std::string& directory) {
  ...
  savePCDFile(directory + "/cloud.pcd", *cloud);
}
...
bool KeyFrame::load(const std::string& directory, g2o::HyperGraph* graph) {
  ...
  if(node_id < 0) {
    LOG_ERROR("invalid node id!!, directory {}", directory);
    return false;
  }

  if(graph->vertices().find(node_id) == graph->vertices().end()) {
    LOG_ERROR("vertex ID={} does not exist!!", node_id);
    return false;
  }

  node = dynamic_cast<g2o::VertexSE3*>(graph->vertices()[node_id]);
  if(node == nullptr) {
    LOG_ERROR("failed to downcast!!");
    return false;
  }
...
KeyFrameSnapshot::KeyFrameSnapshot(const Eigen::Isometry3d& pose, const pcl::PointCloud<PointT>::ConstPtr& cloud, const int& id) : pose(pose), cloud(cloud), id(id) {}

KeyFrameSnapshot::KeyFrameSnapshot(const KeyFrame::Ptr& key) : pose(key->node->estimate()), cloud(key->cloud), id(key->node->id()) {}
