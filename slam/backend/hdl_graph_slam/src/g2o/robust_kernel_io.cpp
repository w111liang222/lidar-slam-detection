...
bool load_robust_kernels(const std::string& filename, g2o::SparseOptimizer* graph) {
  ...
  if(!ifs) {
    LOG_WARN("warning: failed to open input stream!!");
    return true;
  }
  ...
  if(kernels.size() != 0) {
    LOG_WARN("warning: there is non-associated kernels!!");
  }
  return true;
}
