...
template <typename PointTarget, typename PointSource>
void LsqRegistration<PointTarget, PointSource>::setMaxProcessTime(int64_t time) {
  lm_timeout_ = time / 1000;
}
...
bool LsqRegistration<PointTarget, PointSource>::is_converged(const Eigen::Isometry3d& delta) const {
  double R = Eigen::AngleAxisd(delta.linear()).angle() / M_PI * 180;
  Eigen::Vector3d t = delta.translation();

  double r_delta = 1.0 / rotation_epsilon_ * R;
  Eigen::Vector3d t_delta = 1.0 / transformation_epsilon_ * t.array().abs();

  return std::max(r_delta, t_delta.maxCoeff()) < 1;
}

template <typename PointTarget, typename PointSource>
bool LsqRegistration<PointTarget, PointSource>::is_converged_low(const Eigen::Isometry3d& delta) const {
  double R = Eigen::AngleAxisd(delta.linear()).angle() / M_PI * 180;
  Eigen::Vector3d t = delta.translation();

  double r_delta = 1.0 / (rotation_epsilon_ * 10) * R;
  Eigen::Vector3d t_delta = 1.0 / (transformation_epsilon_ * 10) * t.array().abs();
