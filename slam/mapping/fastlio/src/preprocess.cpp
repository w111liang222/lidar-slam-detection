...
Preprocess::Preprocess()
  :feature_enabled(0), blind(0.01), point_filter_num(1), max_point_num(-1)
{
  inf_bound = 10;
  N_SCANS   = 6;
  SCAN_RATE = 10;
...
void Preprocess::process(PointCloudAttrPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  // switch (time_unit)
  // {
  //   case SEC:
...
void Preprocess::velodyne_handler(PointCloudAttrPtr &pl_orig)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    int plsize = pl_orig->cloud->points.size();
    if (plsize == 0) return;
    pl_surf.reserve(plsize);
    ...
    // {
      if (max_point_num > 0) {
        point_filter_num = std::max(1, plsize / max_point_num);
      }
      for (int i = 0; i < plsize; i++)
      {
        PointType added_pt;
        ...
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.x = pl_orig->cloud->points[i].x;
