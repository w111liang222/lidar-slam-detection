...
void OdomEstimationClass::updatePointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr& surf_in, const Eigen::Matrix4d& init_guess){

    if(optimization_count>2)
        optimization_count--;

    Eigen::Isometry3d odom_prediction;
    if (init_guess.isIdentity()) {
        odom_prediction = odom * (last_odom.inverse() * odom);
    } else {
        odom_prediction = odom * init_guess;
    }
    last_odom = odom;
