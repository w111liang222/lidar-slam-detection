    ...
    if ref_boxes is not None and len(ref_boxes) > 0:
        ...
        if ref_labels is None:
            ...
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                ...
                mask = (ref_labels == k)
                fig = draw_corners3d(ref_corners3d[mask], fig=fig, color=cur_color, cls=ref_scores[mask], tag=class_names[k-1], max_num=100)
    ...
    for n in range(num):
        ...
        if cls is not None:
            if isinstance(cls, np.ndarray):
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%s %.2f' % (tag, cls[n]), scale=(0.3, 0.3, 0.3), color=color, figure=fig)
            else:
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%s %s' % (tag, cls[n]), scale=(0.3, 0.3, 0.3), color=color, figure=fig)
