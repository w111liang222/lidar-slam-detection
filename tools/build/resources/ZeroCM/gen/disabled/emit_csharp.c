...
void setup_csharp_options(getopt_t *gopt)
{
    ...
    getopt_add_string(gopt, 0,   "csharp-decl",        ": ZCM.ZCM.ZCMEncodable", "String added to class declarations");
    getopt_add_string(gopt, 0,   "csharp-root-nsp",    "",						 "Root C#.NET namespace (wrapper) added before ZCM package");
    getopt_add_string(gopt, 0,   "csharp-default-nsp", "ZCMTypes",               "Default C#.NET namespace if ZCM type has no package");
}
...
const char *make_fqn_csharp(zcmgen_t *zcm, const char *type_name)
{
	char *root_nsp = getopt_get_string(zcm->gopt, "csharp-root-nsp");

    if (strchr(type_name, '.') != NULL)
        return g_strdup_printf("%s%s%s", root_nsp, (root_nsp[0] == 0 ? "" : "."), type_name);

    if (!ndefaultpkg_warned && !getopt_was_specified(zcm->gopt, "csharp-default-nsp")) {
        printf("Notice: enclosing ZCM types without package into C#.NET namespace '%s'.\n", getopt_get_string(zcm->gopt, "csharp-default-nsp"));
        ndefaultpkg_warned = 1;
    }

	char *def_nsp = getopt_get_string(zcm->gopt, "csharp-default-nsp");
	if (strlen(def_nsp) > 0)
		return g_strdup_printf("%s%s%s.%s", root_nsp, (root_nsp[0] == 0 ? "" : "."), getopt_get_string(zcm->gopt, "csharp-default-nsp"), type_name);
	else
...
int emit_csharp(zcmgen_t *zcm)
{
    ...
    for (unsigned int en = 0; en < g_ptr_array_size(zcm->enums); en++) {
        ...
        const char *classname = make_fqn_csharp(zcm, le->enumname->lctypename);
        char *path = g_strdup_printf("%s%s%s.cs", 
                                  getopt_get_string(zcm->gopt, "csharp-path"),
                                  strlen(getopt_get_string(zcm->gopt, "csharp-path")) > 0 ? G_DIR_SEPARATOR_S : "",
								  dots_to_slashes((getopt_get_bool(zcm->gopt, "csharp-strip-dirs") ? le->enumname->lctypename : classname)));

        if (!zcm_needs_generation(zcm, le->zcmfile, path))
            continue;

        if (getopt_get_bool(zcm->gopt, "csharp-mkdir"))
        ...
        emit(0, "{");
        emit(1, "public sealed class %s %s", le->enumname->shortname, getopt_get_string(zcm->gopt, "csharp-decl"));
        ...
        emit(2,"public void _encodeRecursive(ZCMDataOutputStream outs)");
        emit(2,"{");
        emit(3,"outs.WriteInt(this.value);");
        emit(2,"}");
        emit(0," ");

        emit(2,"public void Encode(ZCMDataOutputStream outs)");
        emit(2,"{");
        emit(3,"outs.Write((long) ZCM_FINGERPRINT);");
        emit(3,"_encodeRecursive(outs);");
        emit(2,"}");
        emit(0," ");

        emit(2,"public static %s _decodeRecursiveFactory(ZCMDataInputStream ins)", make_fqn_csharp(zcm, le->enumname->lctypename));
        emit(2,"{");
        emit(3,"%s o = new %s(0);", make_fqn_csharp(zcm, le->enumname->lctypename), make_fqn_csharp(zcm, le->enumname->lctypename));
        emit(3,"o._decodeRecursive(ins);");
        emit(3,"return o;");
        emit(2,"}");
        emit(0," ");

        emit(2,"public void _decodeRecursive(ZCMDataInputStream ins)");
        emit(2,"{");
        emit(3,"this.value = ins.ReadInt();");
        emit(2,"}");
        emit(0," ");

        emit(2,"public %s(ZCMDataInputStream ins)", le->enumname->shortname);
        emit(2,"{");
        emit(3,"ulong hash = (ulong) ins.ReadInt64();");
        emit(3,"if (hash != ZCM_FINGERPRINT)");
        emit(4,     "throw new System.IO.IOException(\"ZCM Decode error: bad fingerprint\");");
        ...
        emit(2,"{");
        emit(3,"return ZCM_FINGERPRINT;");
        emit(2,"}");
        emit(0," ");
        emit(2, "public static const ulong ZCM_FINGERPRINT = 0x%016"PRIx64"L;", le->hash);
        emit(1, "}");
        emit(0, "}");
        fclose(f);
    }
    ...
    for (unsigned int st = 0; st < g_ptr_array_size(zcm->structs); st++) {
        ...
        const char *classname = make_fqn_csharp(zcm, lr->structname->lctypename);
        char *path = g_strdup_printf("%s%s%s.cs", 
                                  getopt_get_string(zcm->gopt, "csharp-path"), 
                                  strlen(getopt_get_string(zcm->gopt, "csharp-path")) > 0 ? "/" : "",
								  dots_to_slashes((getopt_get_bool(zcm->gopt, "csharp-strip-dirs") ? lr->structname->lctypename : classname)));
        ...
        emit(0, "{");
        emit(1, "public sealed class %s %s", lr->structname->shortname, getopt_get_string(zcm->gopt, "csharp-decl"));
        emit(1, "{");
        ...
        for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
            ...
            if (pinfo==NULL)  {
                emit_continue("%s", make_fqn_csharp(zcm, lm->type->lctypename));
            } else {
        ...
        emit(2, "public static readonly ulong ZCM_FINGERPRINT;");
        emit(2, "public static readonly ulong ZCM_FINGERPRINT_BASE = 0x%016"PRIx64"L;", lr->hash);
        emit(0," ");
        ...
        emit(2, "{");
        emit(3, "ZCM_FINGERPRINT = _hashRecursive(new List<String>());");
        emit(2, "}");
        emit(0, " ");

        emit(2, "public static ulong _hashRecursive(List<String> classes)");
        emit(2, "{");
        emit(3, "if (classes.Contains(\"%s\"))", make_fqn_csharp(zcm, lr->structname->lctypename));
        emit(4,     "return 0L;");
        emit(0, " ");
        emit(3, "classes.Add(\"%s\");", make_fqn_csharp(zcm, lr->structname->lctypename));

        emit(3, "ulong hash = ZCM_FINGERPRINT_BASE");
        for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
            zcm_member_t *lm = (zcm_member_t *) g_ptr_array_index(lr->members, member);
            primitive_info_t *pinfo = (primitive_info_t*) g_hash_table_lookup(type_table, lm->type->lctypename);
            ...
            emit(4, " + %s._hashRecursive(classes)", make_fqn_csharp(zcm, lm->type->lctypename));
        }
        emit(4,";");
        ...
        emit(2, "public %s(byte[] data) : this(new ZCMDataInputStream(data))", lr->structname->shortname);
        emit(2, "{");
        emit(2, "}");
        emit(0, " ");

        emit(2,"public %s(ZCMDataInputStream ins)", lr->structname->shortname);
        emit(2,"{");
        emit(3,"if ((ulong) ins.ReadInt64() != ZCM_FINGERPRINT)");
        emit(4,     "throw new System.IO.IOException(\"ZCM Decode error: bad fingerprint\");");
        emit(0," ");
        emit(3,"_decodeRecursive(ins);");
        emit(2,"}");
        ...
        emit(2,"public static %s _decodeRecursiveFactory(ZCMDataInputStream ins)", make_fqn_csharp(zcm, lr->structname->lctypename));
        emit(2,"{");
        emit(3,"%s o = new %s();", make_fqn_csharp(zcm, lr->structname->lctypename), make_fqn_csharp(zcm, lr->structname->lctypename));
        emit(3,"o._decodeRecursive(ins);");
        emit(3,"return o;");
        emit(2,"}");
        emit(0," ");
        ...
        for (unsigned int member = 0; member < g_ptr_array_size(lr->members); member++) {
            ...
            if (pinfo != NULL) 
                ...
                emit_continue("%s = %s._decodeRecursiveFactory(ins);", accessor, make_fqn_csharp(zcm, lm->type->lctypename));
            }
            emit_end("");
