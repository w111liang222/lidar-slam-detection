...
//static int
//is_same_package (const zcm_typename_t *tn1, const zcm_typename_t *tn2) {
...
_emit_decode_one (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls,
        zcm_member_t *lm, const char *accessor, int indent, const char *sfx)
{
    const char *tn = lm->type->lctypename;
...
_emit_decode_list(const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls,
        zcm_member_t *lm, const char *accessor, int indent, int is_first,
...
static void
emit_js_decode_one (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
    ...
    for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
        ...
        if (! lm->dimensions->len) {
            if (fmt && strcmp(lm->type->lctypename, "boolean")) {
                ...
                _emit_decode_one (zcm, f, ls, lm, accessor, 2, "");
                g_free(accessor);
            }
        } else {
            _flush_read_struct_fmt (zcm, f, struct_fmt, struct_members);
            ...
            if(zcm_is_primitive_type(lm->type->lctypename) &&
               0 != strcmp(lm->type->lctypename, "string")) {
                ...
                g_string_append_printf (accessor, ".append(");
                _emit_decode_one (zcm, f, ls, lm, accessor->str, n+3, ")");
            }
...
static void
emit_js_decode (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
...
static void
emit_js_encode_one (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
...
static void
emit_js_encode (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
...
static void
emit_js_init (const zcmgen_t *zcm, FILE *f, zcm_struct_t *lr)
{
...
static void
emit_js_fingerprint (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
...
static void
emit_js_dependencies (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
{
...
//            g_mkdir_with_parents (package_dir, 0755);
    ...
    for (int i=0; i<pc->enums->len; i++) {
        ...
        char path[PATH_MAX];
        sprintf (path, "%s%s.js", package_dir, le->enumname->shortname);
        ...
        fprintf(f, "\"\"\"ZCM type definitions\n"
                ...
                "\"\"\"\n"
                "\n");
        fprintf(f, "function stringToUint(string) {\n");
        fprintf(f, "    var string = btoa(unescape(encodeURIComponent(string))),\n");
        fprintf(f, "        charList = string.split(''),\n");
        fprintf(f, "        uintArray = [];\n");
        fprintf(f, "    for (var i = 0; i < charList.length; i++) {\n");
        fprintf(f, "        uintArray.push(charList[i].charCodeAt(0));\n");
        fprintf(f, "    }\n");
        fprintf(f, "    return new Uint8Array(uintArray);\n");
        fprintf(f, "}\n");
        fprintf(f, "\n");
        fprintf(f, "function uintToString(uintArray) {\n");
        fprintf(f, "    var encodedString = String.fromCharCode.apply(null, uintArray),\n");
        fprintf(f, "        decodedString = decodeURIComponent(escape(atob(encodedString)));\n");
        fprintf(f, "    return decodedString;\n");
        fprintf(f, "}\n");
    ...
    for (int i = 0; i<pc->structs->len; i++) {
        ...
        char path[PATH_MAX];
        sprintf (path, "%s%s.js", package_dir, ls->structname->shortname);
        ...
        emit_js_init (zcm, f, ls);
        emit_js_encode (zcm, f, ls);
        emit_js_encode_one (zcm, f, ls);
        emit_js_decode (zcm, f, ls);
        emit_js_decode_one (zcm, f, ls);
        emit_js_fingerprint (zcm, f, ls);
        fclose (f);
    }
